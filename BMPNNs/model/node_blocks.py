...
class AttentionMechanism(nn.Module):
    def __init__(self, input_dim, edge_dim, hidden_dim):
        ...
        self.attn_vector = nn.Parameter(torch.empty(1, hidden_dim))
        nn.init.xavier_uniform_(self.attn_vector)
        self.leaky_relu = nn.LeakyReLU(negative_slope=0.2)
    def forward(self, src, dest, edge_attr, edge_index):
        src_proj = self.W_src(src)
        dest_proj = self.W_dest(dest)
        edge_proj = self.W_edge(edge_attr)
        scores = self.leaky_relu((src_proj + dest_proj + edge_proj) @ self.attn_vector.T)
        _, col = edge_index
...
class ABMPNodeBlock(BMPNodeBlock): 
    ...
    def forward(self, x, edge_index, edge_attr, message):
        ...
        out = torch.cat([forward, backward], dim=1)
        out = self.node_mlp(out)
        importance_scores = self.attention_mlp(out).view(-1)
        return out, importance_scores


class CBMPNodeBlock(BMPNodeBlock):
    def __init__(self, hidden_dim, dropout_rate):
        super().__init__(hidden_dim, dropout_rate)
        self.node_mlp = nn.Sequential(
            nn.Linear(2 * hidden_dim, hidden_dim), 
...
class BMP_SNNodeBlock(BMPNodeBlock):
    ...
    def forward(self, x, edge_index, message):
        ...
        importance_scores = self.attention_mlp(out).view(-1) 
        return out, importance_scores
class ABMP_SNNodeBlock(ABMPNodeBlock):
    def __init__(self, input_dim, edge_dim, hidden_dim, dropout_rate):
        super().__init__()
        self.node_mlp = nn.Sequential(
            nn.Linear(2 * hidden_dim, hidden_dim), 
